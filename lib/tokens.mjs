...
// ---------------------------------------------------------------------------
export function estimateOverhead(currentTokens, transcriptPath) {
	if (!transcriptPath || !currentTokens) return 0;
	try {
		const conversationTokens = Math.round(fs.statSync(transcriptPath).size / 4);
		return Math.max(0, currentTokens - conversationTokens);
	} catch {
		return 0;
	}
}
