...
import { compactMessages, detectProjectRoot } from "./compact-output.mjs";
...
function extractMessages(lines, startIdx) {
	...
	// Detect project root from tool_use file paths for path stripping
	const projectRoot = detectProjectRoot(lines, startIdx);
...
export function extractConversation(transcriptPath) {
	...
	const merged = compactMessages(messages);
