...
function extractMessages(lines, startIdx) {
	...
	for (let i = startIdx; i < lines.length; i++) {
		...
		if (obj.type === "assistant" && obj.message?.role === "assistant") {
			...
			if (processed && !isSyntheticAck(processed)) {
...
const DECISION_RE =
	/\b(decided to|chose .+ over|went with|trade-?off|rationale|design decision|instead of .+ because|pros? and cons?|reject(?:ed|ing) .+ in favo[u]?r)\b/i;
...
function compressColdAssistant(msg) {
	...
	const limit = DECISION_RE.test(body)
