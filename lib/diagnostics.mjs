...
import { DATA_DIR as DEFAULT_DATA_DIR } from "./paths.mjs";
...
function discoverDataDirs() {
	const dirs = [argPluginData, DEFAULT_DATA_DIR].filter(Boolean);
	const scanRoots = [
...
const DATA_DIR = recentState?.dir || argPluginData || DEFAULT_DATA_DIR;
...
// 7. Statusline configured
// The statusline is auto-configured by session-start.mjs. If configured but
// not yet visible, it means this is the first session after install — it will
// appear on next restart since Claude Code reads settings at startup.
try {
	...
	if (fs.existsSync(settingsPath)) {
		...
		if (settings.statusLine?.command?.includes("statusline.mjs")) {
			...
			check(
				"statusline",
				true,
				"Another statusline is active — CG statusline not installed to avoid conflict",
			);
		} else {
			check(
				"statusline",
