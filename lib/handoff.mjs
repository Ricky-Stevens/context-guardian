 ...
 * @param {string} [opts.projectDir] - Project root for the .context-guardian
 *   artifact dir. Defaults to process.cwd(); callers should pass CC's real cwd
 *   (from session state) since process.cwd() drifts on a Bash `cd` into a subdir.
...
export function performHandoff({
	...
	const checkpointTokens = Math.round(
		Buffer.byteLength(fullContent, "utf8") / 4,
	);
	...
	// Tokens of noise stripped during extraction = preTokens - baseline -
	// what made it into the snapshot. Same calc as the compaction box.
	const stripped = Math.max(0, preTokens - overhead - checkpointTokens);
	const prePct =
		maxTokens > 0 ? ((preTokens / maxTokens) * 100).toFixed(1) : "?";

	const postPayloadBytes = Buffer.byteLength(fullContent, "utf8");

	const fileSizeMB = Math.max(0.1, postPayloadBytes / (1024 * 1024)).toFixed(1);
	const lines = [
		`┌──────────────────────────────────────────────────────────────────────────────────────────────────`,
		`│  Session Handoff`,
		`│`,
		`│  Context usage: ${preTokens.toLocaleString()} tokens (${prePct}%)`,
		`│  Stripped:      ~${stripped.toLocaleString()} tokens of noise`,
		`│`,
		`│  Saved to: ${handoffPath} (${fileSizeMB}MB)`,
		`│`,
		`└──────────────────────────────────────────────────────────────────────────────────────────────────`,
	];
	...
	log(
		`handoff-saved session=${sessionId} file=${handoffPath} pre=${preTokens} stripped=${stripped} checkpoint=${checkpointTokens}`,
	);
