...
/**
 * Retitle a session's JSONL by appending a new custom-title line.
 * CC uses the LAST custom-title entry, so this effectively renames the session
 * without deleting the file — avoiding the race where CC recreates a deleted JSONL.
 */
function retitleSession(jsonlPath, sessionId) {
	const stamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
	const retitledName = `cg-resumed-${stamp}`;
	const entry = JSON.stringify({
		type: "custom-title",
		customTitle: retitledName,
		sessionId,
	});
	fs.appendFileSync(jsonlPath, `${entry}\n`);
	log(`synthetic-session: retitled ${sessionId} to "${retitledName}"`);
}
...
export function writeSyntheticSession({
	...
	// If it's the active session, retitle instead of deleting — CC recreates
	// deleted JSONLs (with the old custom-title), causing duplicate "cg" entries
	// that break `/resume cg`. Retitling keeps the file but renames it so only
	// the new synthetic session matches the "cg" title.
	const prev = manifest[title];
	if (prev) {
		try {
			if (prev.uuid === currentSessionId && fs.existsSync(prev.path)) {
				retitleSession(prev.path, prev.uuid);
			} else {
				fs.unlinkSync(prev.path);
			}
			log(
				`synthetic-session: cleaned previous ${prev.uuid}${prev.uuid === currentSessionId ? " (retitled)" : " (deleted)"}`,
			);
		} catch {
