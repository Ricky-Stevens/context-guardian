...
/**
 * Fallback model id for the synthetic assistant message when the real one
 * can't be read from session state (e.g. legacy state files).
 *
 * Claude Code's `/resume` reconstruction runs a model-family classifier over
 * the last assistant message's `message.model` (e.g. `id.includes("opus-4-8")`).
 * If `message.model` is missing, that classifier dereferences `undefined` and
 * `/resume` crashes with "undefined is not an object (evaluating 'H.includes')".
 * So this must always be a non-empty, recognisable model id.
 */
const DEFAULT_MODEL = "claude-opus-4-8";

/**
 * Normalise a model id for `message.model`. Real CC stores the bare id
 * (`claude-opus-4-8`); session state carries the runtime variant which may
 * include a context-window suffix (`claude-opus-4-8[1m]`). Strip the suffix
 * and fall back to DEFAULT_MODEL when unset.
 */
function normalizeModel(model) {
	const id = (model || "").trim().replace(/\[[^\]]*\]\s*$/, "");
 ...
 * @param {string} opts.projectCwd - Absolute path to project root; populates the
 *   synthetic messages' `cwd` field.
 * @param {string} [opts.sessionsDir] - Authoritative session directory to write
 *   into. When provided (derived from the current session's transcript path),
 *   it overrides the value computed from `projectCwd`. This is critical: CC's
 *   `/resume` searches the directory the session actually lives in, which is
 *   NOT necessarily `sanitizeCwd(process.cwd())` — a Bash `cd` into a subdir
 *   before compaction drifts `process.cwd()` and lands the synthetic in the
 *   wrong project dir, breaking `/resume`.
 * @param {string} [opts.model] - Model id from session state; populates the
 *   assistant message's `message.model` so CC's resume model classifier works.
...
export function writeSyntheticSession({
	...
	const resolvedSessionsDir = sessionsDir || getSessionsDir(projectCwd);
	...
	// title collision with CC's in-memory cache. Handoff entries are never
	// cleaned up by compaction. Scope cleanup to THIS project's session dir:
	// a compact synthetic in another directory belongs to a concurrent session
	// (the user often runs several at once) that may still want to resume it.
	if (type === "compact") {
		for (const [key, entry] of Object.entries(manifest)) {
			if (entry.type !== "compact") continue;
			if (path.dirname(entry.path) !== resolvedSessionsDir) continue;
			try {
	...
	const assistantMsg = {
		...
		message: {
			role: "assistant",
			// `model` is required: CC's /resume reconstruction runs a model-family
			// classifier over the last assistant message's model id. A missing
			// model crashes /resume with "undefined is not an object".
	...
	fs.mkdirSync(resolvedSessionsDir, { recursive: true });
	const jsonlPath = path.join(resolvedSessionsDir, `${sessionUuid}.jsonl`);
