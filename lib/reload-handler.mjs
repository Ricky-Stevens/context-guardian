...
const INJECTION_FRAMING =
	...
	"1. The '## Conversation Index' at the top is your reference map. ALWAYS check it first when answering questions about the session.\n" +
	"2. Exchange numbers [N] in the index correspond to exchanges in the chronological body below.\n" +
	"3. If the index mentions a fact, it IS in the full record — scan the body at that exchange number for details.\n" +
	"4. The LAST exchanges reflect the current state. Earlier exchanges are context.\n" +
	"5. You have NOT read any files in this session — re-read any file before referencing its contents or making edits.\n" +
...
function injectCheckpoint(
	...
	if (isResumeNow && hasOriginal) {
		...
		output({
			hookSpecificOutput: {
				hookEventName: "UserPromptSubmit",
				additionalContext: `${historyBlock}\n\nIMPORTANT: The user just cleared context and this checkpoint was auto-restored. Before responding to the user's message, you MUST first confirm the restore by showing the compaction stats above and saying context has been restored.${resumeHint}`,
			},
		});
	}
