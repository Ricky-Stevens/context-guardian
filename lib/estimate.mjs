...
/** Maximum bytes to read from the transcript (same as transcript.mjs). */
...
export function estimateSavings(transcriptPath, currentTokens, maxTokens) {
	...
	try {
		...
		const smartTokens = Math.round(currentTokens * smartKeepRatio);
		...
		const recentTokens = Math.round(smartTokens * recentRatio);
...
// ---------------------------------------------------------------------------
// Transcript reader (lightweight copy — avoids importing transcript.mjs)
// ---------------------------------------------------------------------------

/**
 * Read transcript lines with a memory cap.
 * Mirrors readTranscriptLines in transcript.mjs but is self-contained
