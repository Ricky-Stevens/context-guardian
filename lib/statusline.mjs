...
/**
 * Persist the authoritative context_window_size from Claude Code's statusline
 * JSON into the per-session state file. The statusline is the only CG component
 * that receives this value directly from CC. Hooks read it from the state file
 * and carry it forward on subsequent writes.
 */
/**
...
function render(data) {
	...
	// Compute threshold — adaptive based on context window size, unless the
	// user explicitly set one via /cg:config.
	const dataDir =
		process.env.CLAUDE_PLUGIN_DATA || path.join(os.homedir(), ".claude", "cg");
	let threshold;
	let userSetThreshold = false;
	try {
		const configPath = path.join(dataDir, "config.json");
		if (fs.existsSync(configPath)) {
			const cfg = JSON.parse(fs.readFileSync(configPath, "utf8"));
			if ("threshold" in cfg) {
				threshold = Math.round(cfg.threshold * 100);
	...
	if (!userSetThreshold) {
		...
		// Same formula as computeAdaptiveThreshold in config.mjs:
		// 55% at 200K, 30% at 1M, clamped [25%, 55%]
		const adaptive = Math.min(
			0.55,
			Math.max(0.25, 0.55 - ((windowSize - 200000) * 0.25) / 800000),
		);
		threshold = Math.round(adaptive * 100);
	}
