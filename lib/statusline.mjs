...
import { resolveStatuslineStateDir } from "./paths.mjs";
 ...
 */
export function persistSessionMetadata(data) {
	const size = data.context_window?.context_window_size;
	const modelId = data.model?.id;
	const sessionId = data.session_id;
	if (!sessionId || (typeof size !== "number" && !modelId)) return;
	try {
		const stateDir = resolveStatuslineStateDir();
		fs.mkdirSync(stateDir, { recursive: true });
		const filePath = path.join(stateDir, `state-${sessionId}.json`);
...
// ---------------------------------------------------------------------------
export function resolveThreshold(data) {
	// Config does live in CLAUDE_PLUGIN_DATA (per-install), so this lookup
	// stays env-aware.
	const dataDir = process.env.CLAUDE_PLUGIN_DATA || resolveStatuslineStateDir();
	try {
...
// ---------------------------------------------------------------------------
// Session size — reads the current session's state file for payload_bytes
// and baseline_overhead. Multiple concurrent CC sessions write to the same
// dir, so picking by mtime can return another session's data; always look
// up by session_id when available.
// ---------------------------------------------------------------------------
export function readSessionSize(dataDir, sessionId) {
	let target;
	if (sessionId) {
		const candidate = path.join(dataDir, `state-${sessionId}.json`);
		if (fs.existsSync(candidate)) target = candidate;
		else return 0;
	} else {
		// Fallback for older CC versions that don't pipe session_id.
		const stateFiles = fs
			.readdirSync(dataDir)
		...
		target = path.join(dataDir, newest);
	}
...
// ---------------------------------------------------------------------------
export function formatSessionSize(totalBytes, dim, reset) {
...
export function render(data) {
	...
	try {
		sessionStr = formatSessionSize(
			readSessionSize(resolveStatuslineStateDir(), data.session_id),
			dim,
			reset,
		);
	} catch {}
...
/**
 * Parse a raw statusline stdin payload, persist session metadata, and render
 * the status bar string. Returns the fallback marker on any parse/render
 * failure. Exported so the CLI path can be unit-tested in-process (the
 * spawn-based tests don't register against coverage instrumentation).
 *
 * @param {string} raw - Raw JSON payload from Claude Code on stdin
 * @returns {string} Rendered status line, or "Context: --" on failure
 */
export function renderStatusline(raw) {
	try {
...
// ---------------------------------------------------------------------------
// CLI entry point — only run the stdin handler when invoked as a script,
// not when imported by tests for in-process unit testing.
// ---------------------------------------------------------------------------
const isCli = process.argv[1]?.endsWith("statusline.mjs");
if (isCli) {
	let raw = "";
