...
// Fixed location the statusline reads from. The statusline process doesn't
// receive CLAUDE_PLUGIN_DATA reliably (and a dev-override settings.json can
// even point it at the wrong dir), so we always write a copy here for it to
// pick up. CG_STATE_DIR overrides the location in tests so they don't pollute
// the user's real ~/.claude/cg/. Resolved at call time so tests that set the
// env after `paths.mjs` is first imported still take effect.
export function resolveStatuslineStateDir() {
	return process.env.CG_STATE_DIR || path.join(os.homedir(), ".claude", "cg");
}

/** @deprecated Use resolveStatuslineStateDir() for call-time resolution. */
export const STATUSLINE_STATE_DIR = resolveStatuslineStateDir();
