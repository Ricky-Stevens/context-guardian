...
1. **Token state** — `${CLAUDE_PLUGIN_DATA}/state.json`
   Contains: `current_tokens`, `max_tokens`, `pct`, `session_id`, `ts`
...
│  Data source:     real counts / unavailable
│
...
- If state.json is missing/stale: "No recent data. Send a message first so the Stop hook can capture token counts."
