...
describe("generateConversationIndex", () => {
  ...
  it("returns empty string when no exchanges produce facts or work", () => {
    ...
    const result = generateConversationIndex(messages);
    assert.equal(result, "", "Generic user/assistant pairs without entities or tools should produce empty index");
  });
  ...
  it("generates index entries for exchanges with tool work", () => {
    ...
    assert.ok(result.includes("## Conversation Index"), "Should have index header");
    assert.ok(result.includes("Compact reference"), "Should have description line");
  ...
  it("generates index entries for entity-rich user messages without tools", () => {
    ...
    assert.ok(result.includes("ZEP-"), "Should include ticket IDs from user messages");
  ...
  it("includes decisions section when user messages contain decision patterns", () => {
    ...
    assert.ok(result.includes("rejected"), "Should include 'rejected' decision");
  ...
  it("includes error-resolution pairs", () => {
    ...
    const result = generateConversationIndex(messages);
    assert.notEqual(result, "");
    assert.ok(result.includes("**Errors resolved:**"), "Should have error resolution section");
    assert.ok(result.includes("Module not found"), "Should include the error text");
  ...
  it("truncates long facts to 250 chars and appends entity tags", () => {
    ...
    assert.ok(longLine.includes("..."), "Truncated line should end with ellipsis");
    ...
    assert.ok(!longLine.includes("A".repeat(300)), "Should not contain full 300-char string");
  });
  ...
  it("entity extraction pulls IDs, money, dates, and ports into index entries", () => {
    ...
    assert.ok(result.includes("$184,000"), "Should include money amount $184,000");
    assert.ok(result.includes("port 5433"), "Should include port reference");
    assert.ok(result.includes("PROJ-9999"), "Should include ticket ID PROJ-9999");
  });
  ...
  it("handles mixed content with tools, decisions, errors, and entities", () => {
    ...
    assert.ok(result.includes("**Decisions:**"), "Should have decisions section");
    assert.ok(result.includes("**Errors resolved:**"), "Should have error resolution section");
    assert.ok(result.includes("PROJ-2000"), "Should include entity from first exchange");
  });
});
...
describe("addSectionHeaders", () => {
  ...
  it("inserts section headers every 10 exchanges for 25+ exchange sessions", () => {
    ...
    assert.ok(result.length > messages.length, "Should have added header elements");
    // Find all section headers
    const headers = result.filter((m) => m.startsWith("### Exchanges"));
    assert.ok(headers.length >= 2, `Should have at least 2 section headers, got ${headers.length}`);
    // Check header format
    for (const header of headers) {
      assert.match(header, /^### Exchanges \d+-\d+$/, `Header should match format, got: ${header}`);
    }
  });
  ...
  it("uses default groupSize when not specified", () => {
    ...
    const result = addSectionHeaders(messages);
    const headers = result.filter((m) => m.startsWith("### Exchanges"));
    assert.ok(headers.length >= 1, "Should insert headers with default groupSize");
  });
  ...
  it("handles messages with tool lines between exchanges", () => {
    ...
    const headers = result.filter((m) => m.startsWith("### Exchanges"));
    assert.ok(headers.length >= 1, `Should have section headers, got ${headers.length}`);
    // All original messages should still be present
    for (const msg of messages) {
      assert.ok(result.includes(msg), `Original message should be preserved: ${msg.substring(0, 50)}`);
    }
  });
